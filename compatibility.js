...
function compatibilityScore(man,woman){
 ...
 const parts=[{icon:'faces',label:'日主互動',value:dayMaster,max:30,note:man.day+' × '+woman.day},{icon:'link',label:'日支相處',value:branchPoints[dayRelation],max:30,note:dayRelation},{icon:'sun',label:'月支節奏',value:monthPoints[monthRelation],max:20,note:monthRelation},{icon:'flower',label:'五行互補',value:complement,max:20,note:'雙方強弱元素對照'}];
 return{total:parts.reduce(function(sum,part){return sum+part.value},0),parts:parts};
}
const compatibilityScoreIcons={
 heart:'<svg viewBox="0 0 80 80" aria-hidden="true"><path d="M40 68C26 58 10 45 10 28c0-10 7-17 16-17 6 0 11 3 14 8 3-5 8-8 14-8 9 0 16 7 16 17 0 17-16 30-30 40Z" fill="#c85f52"/><circle cx="31" cy="35" r="3" fill="#fff5e8"/><circle cx="49" cy="35" r="3" fill="#fff5e8"/><path d="M35 46q5 5 10 0" fill="none" stroke="#fff5e8" stroke-width="3" stroke-linecap="round"/><path d="M8 13l2-5 2 5 5 2-5 2-2 5-2-5-5-2Z" fill="#d6aa57"/><path d="M68 54l2-4 2 4 4 2-4 2-2 4-2-4-4-2Z" fill="#d6aa57"/></svg>',
 faces:'<svg viewBox="0 0 40 40" aria-hidden="true"><circle cx="15" cy="20" r="11" fill="#e6bd76"/><circle cx="27" cy="20" r="10" fill="#d77b68"/><path d="M11 20h.1M18 20h.1M13 25q2 2 4 0M23 20h.1M30 20h.1M25 25q2 2 4 0" fill="none" stroke="#514744" stroke-width="2" stroke-linecap="round"/></svg>',
 link:'<svg viewBox="0 0 40 40" aria-hidden="true"><circle cx="15" cy="20" r="10" fill="none" stroke="#d77b68" stroke-width="4"/><circle cx="26" cy="20" r="10" fill="none" stroke="#d1a253" stroke-width="4"/><path d="M20 10l2-4 2 4 4 2-4 2-2 4-2-4-4-2Z" fill="#d77b68"/></svg>',
 sun:'<svg viewBox="0 0 40 40" aria-hidden="true"><circle cx="20" cy="20" r="9" fill="#e6bd76"/><path d="M20 2v6M20 32v6M2 20h6M32 20h6M7 7l4 4M29 29l4 4M33 7l-4 4M11 29l-4 4" stroke="#c58855" stroke-width="2.5" stroke-linecap="round"/><path d="M16 21h.1M24 21h.1M18 25q2 2 4 0" fill="none" stroke="#755239" stroke-width="2" stroke-linecap="round"/></svg>',
 flower:'<svg viewBox="0 0 40 40" aria-hidden="true"><circle cx="20" cy="10" r="7" fill="#d9ad77"/><circle cx="30" cy="19" r="7" fill="#d77b68"/><circle cx="26" cy="30" r="7" fill="#8ab098"/><circle cx="14" cy="30" r="7" fill="#8aa9bb"/><circle cx="10" cy="19" r="7" fill="#d4bf83"/><circle cx="20" cy="20" r="6" fill="#fff1d5"/><path d="M18 20h.1M22 20h.1M19 23q1 1 2 0" fill="none" stroke="#695b4d" stroke-width="1.5" stroke-linecap="round"/></svg>'
};
function compatibilityScoreMarkup(score){
 return '<div class="compat-score"><div class="compat-score-main"><div class="compat-score-hero">'+compatibilityScoreIcons.heart+'<div><span>三柱互動參考分</span><strong>'+score.total+'<small> / 100</small></strong></div></div><p>滿分 100，僅依年、月、日三柱的自訂規則計算；未納入時柱。不是交往成功率，也不代表關係好壞。</p></div><div class="compat-score-parts">'+score.parts.map(function(part){return '<div class="compat-score-part"><div><span class="compat-score-icon">'+compatibilityScoreIcons[part.icon]+'</span><b>'+part.label+'</b><small>'+compatibilityEscape(part.note)+'</small><strong>'+part.value+' / '+part.max+'</strong></div><span class="compat-score-track"><i style="width:'+(part.value/part.max*100)+'%"></i></span></div>'}).join('')+'</div></div>';
}
